...
```bash
python3 ./skills/seo-image-gen/scripts/validate_setup.py
```

Or check manually:
1. `ls ./skills/seo-image-gen/SKILL.md`:skill file exists
2. `ls ./agents/seo-image-gen.md`:agent file exists
3. `grep nanobanana ./.mcp.json`:MCP configured
...
- Check `./.mcp.json` has the nanobanana-mcp entry
